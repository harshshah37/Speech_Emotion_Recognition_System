...
# page settings
st.set_page_config(page_title="IPD_Project", page_icon=":speech_balloon:", layout="wide")
...
def get_title(predictions, categories=CAT6):
    title = f"{categories[predictions.argmax()]}"
    return title
...
def main():
    side_img = Image.open("images/emotions.jpg")
    with st.sidebar:
        st.image(side_img, width=300)
    st.sidebar.subheader("Menu")
    website_menu = st.sidebar.selectbox("", ("Emotion Recognition", "Project description", "Our team",
                                                 "Feedback", "Relax"))
    ...
    if website_menu == "Emotion Recognition":
                ...
                with st.container():
                    ...
                    with col1:
                        if em3:
                            ...
                            # title=txt, colors=COLORS)
                            st.write(fig)
                            st.subheader("Detected Emotion:"+txt)
                    with col2:
                        if em6:
                            txt = "Detected Emotion:" + get_title(pred, CAT6)
                            RAT1=get_title(pred, CAT6)
                    ...
                    with col3:
                        if gender:
                            with st.spinner('Wait for it...'):
                                ...
                                ind = gpred.argmax()
                                txt1 = "Predicted gender: " + gdict[ind][0]
        ...
        txt = """
            This web-application is a part of the IPD Project. 
        ...
        with col1:
            st.info("yashshahk1412@gmail.com")
            st.info("agarwalharsh453@gmail.com")
            st.info("deepshah7977@gmail.com")
        with col2:
            liimg = Image.open("images/LI-Logo.png")
            st.image(liimg)
            st.markdown(f""":speech_balloon: [Yash Shah](https://www.linkedin.com/in/yash-shah-70b812213/)""",
                        unsafe_allow_html=True)
            st.markdown(f""":speech_balloon: [Harsh Agarwal](https://www.linkedin.com/in/harsh-agarwal-5794a8214/)""",
                        unsafe_allow_html=True)
            st.markdown(f""":speech_balloon: [Deep Shah](https://www.linkedin.com/in/deepshah08/)""",
                        unsafe_allow_html=True)

    elif website_menu == "Feedback":
        st.subheader("Feedback")
        user_input = st.text_area("Your feedback is greatly appreciated")
        user_name = st.selectbox("Choose your personality", ["demo1", "demo2", "demo3", "demo4"])
